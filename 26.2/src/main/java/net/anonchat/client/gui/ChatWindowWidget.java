...
public final class ChatWindowWidget {
    ...
    public void render(final GuiGraphicsExtractor context) {
        this.render(context, false);
    }

    public void render(final GuiGraphicsExtractor context, final boolean focused) {
        ...
        if (focused) {
            ...
            for (final ChatTabWidget tw : tabWidgets) {
                ...
                final int twidth = font.width(tw.getTab().getName()) + 8;
            ...
            context.fill(bx, by, bx + bw, by + bh, lockCol);
            final int sw = 2;
            final int shackleTop = by - 3;
            if (locked) {
                context.fill(bx + 1, shackleTop, bx + 1 + sw, by, lockCol);
                context.fill(bx + bw - 1 - sw, shackleTop, bx + bw - 1, by, lockCol);
        ...
        // ── Resize handle (Unicode icon instead of yellow square) ──
        if (focused) {
            final Font font = Minecraft.getInstance().font;
            final int iconX = x + width - font.width(RESIZE_ICON) - 2;
            final int iconY = y + height - font.lineHeight - 1;
    ...
    private int getDropdownX() {
        final Minecraft mc = Minecraft.getInstance();
        final int sw = mc.getWindow() != null ? mc.getWindow().getGuiScaledWidth() : 0;
        final boolean roomRight = x + width + DROPDOWN_WIDTH + 6 < sw;
        final boolean roomLeft = x > DROPDOWN_WIDTH + 2;
        if (roomRight) return x + width + 2;       // outside on the right
        if (roomLeft)  return x - DROPDOWN_WIDTH - 2; // outside on the left
        return x + width - DROPDOWN_WIDTH - 4;      // fallback: inside window
    }
    ...
    private void renderDropdown(final GuiGraphicsExtractor context) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            final String label = isSettings
                ? "> " + Component.translatable("key.anonchat.dropdown." + items[i]).getString()
                : Component.translatable("key.anonchat.dropdown." + items[i]).getString();
            if (isSettings) {
            ...
            context.text(font, label, ddX + 4, iy + (DROPDOWN_ITEM_HEIGHT - font.lineHeight) / 2,
    ...
    public boolean clickDropdown(final double mx, final double my) {
        ...
        switch (items[index]) {
            ...
            case "settings":
                Minecraft.getInstance().setScreenAndShow(new AnonChatConfigScreen(null));
                break;
        }
        return true;
    }
    ...
    private void deleteActiveTab() {
        ...
        refreshTabs();
        // Auto-remove empty windows from config (no orphaned JSON entries)
    ...
    public void commitPosition() {
        ...
        // Snap to screen edges (but allow window overlap)
        final int snapPx = 10;
        if (x < snapPx) x = 0;
