...
public class ChatTabImpl extends ChatTab {
    ...
    private static SoundEvent getSoundById(final String id) {
        if ("pling".equals(id)) return SoundEvents.BLOCK_NOTE_BLOCK_PLING.value();
        if ("bell".equals(id))  return SoundEvents.BLOCK_NOTE_BLOCK_BELL.value();
        if ("chime".equals(id)) return SoundEvents.BLOCK_AMETHYST_BLOCK_CHIME;
        if ("xporb".equals(id)) return SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP;
        if ("click".equals(id)) return SoundEvents.UI_BUTTON_CLICK.value();
        return null;
    }
